...
class ViewAdapterBlogHomeDetail(var ds:List<DataView>, val onClick:RvInterface): RecyclerView.Adapter<ViewAdapterBlogHomeDetail.Detail>() {
    ...
    inner class Detail(itemView: View): RecyclerView.ViewHolder(itemView){
        init {
            imageBlog = itemView.findViewById(R.id.img_blog_home_detail)
            txtTitleBlog = itemView.findViewById(R.id.txtTitleBlog_home_detail)
            txtContentBlog = itemView.findViewById(R.id.txtContent_home_detail)
            txtUsernameBlog = itemView.findViewById(R.id.txtUsernameBlog_home_detail)
            txtTimeBlog = itemView.findViewById(R.id.txtTimeBlog_home_detail)
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Detail {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.layout_blog_home_detail,parent,false)
        return  Detail(view)
    }
    ...
    override fun onBindViewHolder(holder: Detail, position: Int) {
        holder.itemView.apply {
            ...
            txtUsernameBlog?.text = "Người dùng: ${ds[position].username}"
            txtTimeBlog?.text = "Ngày đăng tải: ${ds[position].timeblog}"
            txtContentBlog?.maxLines = 10

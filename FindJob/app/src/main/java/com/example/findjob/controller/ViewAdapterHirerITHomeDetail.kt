...
class ViewAdapterHirerITHomeDetail(var ds:List<DataView>, val onClick:RvInterface): RecyclerView.Adapter<ViewAdapterHirerITHomeDetail.Detail>() {
    ...
    inner class Detail(itemView: View): RecyclerView.ViewHolder(itemView){
        init {
            imageIt = itemView.findViewById(R.id.img_hired_it_home_detail)
            txtNameJobIt = itemView.findViewById(R.id.txtNameJobb_hired_it_home_detail)
            txtMucLuongIt = itemView.findViewById(R.id.txtMucLuong_hired_it_home_detail)
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Detail {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.layout_hiredit_home_detail,parent,false)
        return  Detail(view)
    }
